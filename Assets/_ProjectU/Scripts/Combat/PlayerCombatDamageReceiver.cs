...
public sealed class PlayerCombatDamageReceiver : MonoBehaviour, ICombatDamageReceiver // 플레이어 전투 피해 수신 관리자
{
    ...
    public bool ReceiveDamage(CombatHitData hitData) // 전투 피해 정보 수신
    {
        ...
        {
            string attackerName = hitData.Attacker == null // 값 계산 시작
                ? "UNKNOWN" // 공격자 없음
                : hitData.Attacker.name; // 공격자 이름 조회
            Debug.Log( // 호출 시작
                $"플레이어 전투 피해 적용 / 공격자 {attackerName} / 피해 {hitData.Damage:0.##} / " // 로그 앞부분
                + $"남은 체력 {playerHealth.CurrentHealth:0.##} / 적용 횟수 {appliedCombatHitCount}", // 로그 뒷부분
                this); // 전투 피해 적용 결과 출력
        }
    ...
    private void LogBlockedResult(CombatHitData hitData, string reason) // 전투 피해 차단 결과 출력
    {
        ...
        string attackerName = hitData.Attacker == null // 값 계산 시작
            ...
            : hitData.Attacker.name; // 공격자 이름 조회
        Debug.Log( // 호출 시작
            $"플레이어 전투 피해 차단 / 사유 {reason} / 공격자 {attackerName} / " // 로그 앞부분
            + $"피해 {hitData.Damage:0.##} / 차단 횟수 {blockedCombatHitCount}", // 로그 뒷부분
            this); // 전투 피해 차단 결과 출력
    }
