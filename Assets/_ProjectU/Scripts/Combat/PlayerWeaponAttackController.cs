...
public sealed class PlayerWeaponAttackController : MonoBehaviour // 플레이어 근접 연속 공격 관리자
{
    ...
    [Tooltip("공격 단계별 비용을 소비할 플레이어 스태미나입니다.")]
    ...
    [Tooltip("한 프레임의 근접 공격에서 확인할 최대 Collider 결과 수입니다.")]
    ...
    public bool IsAttacking => isAttacking; // 현재 공격 진행 여부 제공
    ...
    private void Awake() // 근접 연속 공격 참조 초기화
    {
    ...
    {
        if (!isActiveAndEnabled) // 근접 공격 기능 활성 상태 확인
        {
        ...
        {
            if (logAttackResults) // 원거리 공격 안내 로그 사용 여부 확인
            {
                Debug.Log(
    ...
    public void CancelCurrentAttack() // UI, 건축 모드와 컴포넌트 비활성화 시 공격 즉시 취소
    {
    ...
    private bool TryQueueNextAttack(
        ...
        GatherableResource gatherableTarget) // 현재 공격 중 다음 연속 입력 저장
    {
        ...
        {
            Debug.Log(
                $"{activeAttackProfile.DisplayName} {currentComboStepIndex + 2}단 공격 입력 저장",
        ...
        GatherableResource gatherableTarget) // 근접 공격과 저장된 연속 공격 실행
    {
        ...
        while (true) // 저장된 연속 공격이 끝날 때까지 반복
        {
            ...
            if (!playerStamina.TryConsume(stepStaminaCost)) // 현재 단계 스태미나 소비 시도
            {
                ...
                {
                    Debug.Log(
                        $"스태미나가 부족하여 {attackProfile.DisplayName} {stepIndex + 1}단 공격을 실행할 수 없습니다.",
                        this); // 단계별 스태미나 부족 안내
                }
            ...
            {
                Debug.Log(
                    $"{attackProfile.DisplayName} {stepIndex + 1}단 시작 / {stepData.StepName}",
                    this); // 공격 단계 시작 결과 출력
            }
        ...
        GatherableResource gatherableTarget) // 공격 단계 실행 상태 초기화
    {
        ...
        if (toolSwingAnimation != null) // 휘두르기 연출 존재 확인
        {
            toolSwingAnimation.PlaySwing(
                stepIndex,
                stepData.AnimationSpeedMultiplier); // 공격 단계별 휘두르기 방향과 속도 적용
        }
    }

    private IEnumerator RunWindupPhase(
    ...
    private IEnumerator RunRecoveryPhase(
    ...
    private void PerformMeleeDamageFrame(
        ItemData selectedItem,
        AttackProfile attackProfile,
        MeleeAttackStepData stepData,
        Vector3 attackDirection) // 한 프레임 근접 피해 판정
    {
        ...
        while (damagedTargetCountThisStep < stepData.MaximumTargets) // 남은 피해 대상 수만큼 반복
        {
            ...
            for (int index = 0; index < hitCount; index++) // 전체 충돌 결과 순회
            {
                ...
                if (damageRoot == transform.root
                    || damageRoot.IsChildOf(transform.root)
            ...
            {
                Debug.Log(
                    $"{attackProfile.DisplayName} {currentComboStepIndex + 1}단 명중: "
                    + $"{nearestReceiverComponent.gameObject.name} / 피해 {finalDamage:0.##}",
        ...
        out AttackProfile attackProfile) // 선택 아이템 또는 맨손 공격 능력치 계산
    {
        if (selectedItem != null && selectedItem.CanAttack) // 공격 가능한 아이템 확인
        {
            attackProfile = new AttackProfile(
                ...
                selectedItem.ImpactForce,
                selectedItem.MeleeComboData); // ItemData 전투 능력치와 연속 공격 데이터 복사
        ...
        attackProfile = new AttackProfile(
            ...
            unarmedStaminaCost,
            unarmedImpactForce,
            unarmedComboData); // 맨손 공격 능력치 생성

        return true; // 맨손 공격 능력치 계산 성공
    }

    private int ResolveStartComboStep(
        ItemData selectedItem,
        MeleeComboData comboData) // 새 공격 입력의 시작 연속 공격 단계 계산
    {
        ...
        bool canContinueCombo =
            comboProgressItem == selectedItem
            && comboProgressData == comboData
            && Time.time <= comboProgressExpiresAt
    ...
    private MeleeAttackStepData ResolveStepData(
        MeleeComboData comboData,
        int stepIndex,
    ...
    private void StoreComboProgress(
        ItemData selectedItem,
        MeleeComboData comboData,
        int followingStepIndex) // 현재 공격 종료 후 다음 단계 진행도 저장
    {
        if (comboData == null
            || comboData.StepCount <= 1
            || followingStepIndex <= 0
    ...
    private Vector3 ResolveAttackDirection() // 1·3인칭 화면 중앙 기준 공격 방향 계산
    {
        ...
        Vector3 originToAimPoint =
            aimHit.point - attackOrigin.position; // 공격 시작 위치에서 조준점까지 방향 계산
    ...
    private void OnDrawGizmosSelected() // 근접 공격 범위 시각화
    {
        ...
        Vector3 endPosition =
            attackOrigin.position + direction * previewRange; // 공격 끝 위치 계산
    ...
    {
        CancelCurrentAttack(); // 실행 중인 공격과 연속 공격 진행도 취소
    }
    ...
    private readonly struct AttackProfile // 한 번의 연속 공격에서 사용할 기본 능력치
    {
        public string DisplayName { get; } // 공격 표시 이름
        public WeaponAttackType AttackType { get; } // 공격 방식
        public float Damage { get; } // 기본 피해량
        public float Cooldown { get; } // 연속 공격 데이터가 없을 때 사용할 기본 시간
        ...
        public AttackProfile(
            ...
            float staminaCost,
            float impactForce,
            MeleeComboData comboData) // 기본 공격 능력치 생성
        {
