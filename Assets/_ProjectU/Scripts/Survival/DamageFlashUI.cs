...
public sealed class DamageFlashUI : MonoBehaviour // 공통 피해 화면 효과
{
    [Header("References")] // 외부 참조 묶음
    [Tooltip("플레이어 체력.")]
    [SerializeField] private PlayerHealth playerHealth; // 플레이어 체력
    [Tooltip("피해 화면 이미지.")]
    [SerializeField] private Image flashImage; // 피해 화면 이미지

    [Header("Flash")] // 피해 효과 설정 묶음
    [Tooltip("효과 지속 시간.")]
    [SerializeField] private float flashDuration = 0.25f; // 효과 지속 시간
    [Tooltip("최소 불투명도.")]
    [SerializeField] private float minimumAlpha = 0.15f; // 최소 불투명도
    [Tooltip("최대 불투명도.")]
    [SerializeField] private float maximumAlpha = 0.45f; // 최대 불투명도
    ...
    private void Awake() // 피해 화면 초기화
    {
        if (playerHealth == null || flashImage == null) // 필수 참조 확인
        {
            Debug.LogError($"{gameObject.name}의 피해 화면 참조가 누락되었습니다.", this); // 참조 오류 출력
            enabled = false; // 피해 화면 기능 비활성화
            return; // 초기화 처리 중단
        }
    ...
    private void OnEnable() // 피해 이벤트 연결
    {
        ...
        playerHealth.Damaged += HandleDamaged; // 피해 이벤트 구독
    }

    private void OnDisable() // 피해 이벤트 해제
    {
        if (playerHealth == null) // 체력 참조 확인
        {
            return; // 이벤트 해제 중단
        ...
        playerHealth.Damaged -= HandleDamaged; // 피해 이벤트 구독 해제
    }

    private void Update() // 피해 화면 효과 갱신
    {
    ...
    private void HandleDamaged(float damageAmount) // 피해 화면 표시
    {
        if (damageAmount <= 0f) // 실제 피해량 확인
        {
            return; // 효과 표시 중단
        }

        float damageRatio = Mathf.Clamp01(damageAmount / playerHealth.MaxHealth); // 최대 체력 대비 피해 비율
        flashStartAlpha = Mathf.Lerp(minimumAlpha, maximumAlpha, damageRatio); // 피해량별 불투명도 계산
        flashTimer = flashDuration; // 효과 시간 초기화

        SetFlashAlpha(flashStartAlpha); // 피해 화면 즉시 표시
    }

    private void SetFlashAlpha(float alpha) // 피해 화면 불투명도 적용
    {
    ...
    private void ClampSettings() // 피해 화면 설정값 보정
    {
