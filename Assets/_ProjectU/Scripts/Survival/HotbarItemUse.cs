...
public sealed class HotbarItemUse : MonoBehaviour // 핫바 아이템 사용 처리
{
    ...
    {
        if (playerInventory == null || playerHunger == null || playerThirst == null || popupController == null) // 참조 누락 확인
        {
    ...
    private void TryUseSelectedItem() // 선택 핫바 아이템 사용
    {
        ...
        {
            if (itemData.HungerRestoreAmount <= 0f) // 허기 회복 효과 확인
            {
        ...
        {
            if (itemData.ThirstRestoreAmount <= 0f) // 갈증 회복 효과 확인
            {
                return; // 효과 없는 음료 사용 차단
            }

            bool drinkSucceeded = playerThirst.TryDrink(itemData.ThirstRestoreAmount); // 갈증 회복 시도

            if (!drinkSucceeded) // 음료 사용 결과 확인
            {
                return; // 수량 감소 차단
            }
