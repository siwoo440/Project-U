...
public sealed class GameplaySaveController : MonoBehaviour // 게임 진행 상태 저장과 불러오기 관리
{
    ...
    private void Awake() // 저장 기능 참조 초기화
    {
        ...
        bool hasPlayerTemperature = playerTemperature != null; // 체온 관리기 확인

        if (!hasCharacterController || !hasPlayerMovement || !hasPlayerHealth
            || !hasPlayerHunger || !hasPlayerThirst || !hasPlayerStamina
            || !hasPlayerWetness || !hasPlayerTemperature) // 플레이어 필수 컴포넌트 확인
        {
    ...
    public void SaveCurrentGame() // 현재 게임 상태 저장
    {
        ...
        if (!NpcSaveBridge.TryCapture(saveData, out string npcCaptureError)) // NPC 관계 수집 (91일차)
        {
            Debug.LogError($"NPC 관계 저장 준비 실패\n{npcCaptureError}", this); // 수집 오류 출력
            return; // 파일 저장 중단
        }
        ...
        if (!NpcQuestSaveBridge.TryCapture(saveData, out string npcQuestCaptureError)) // NPC 의뢰 수집 (93일차)
        {
            Debug.LogError($"NPC 의뢰 저장 준비 실패\n{npcQuestCaptureError}", this); // 수집 오류 출력
            return; // 파일 저장 중단
        }

        if (!NpcEventSaveBridge.TryCapture(saveData, out string npcEventCaptureError)) // NPC 이벤트 수집 (94일차)
        {
            Debug.LogError($"NPC 이벤트 저장 준비 실패\n{npcEventCaptureError}", this); // 수집 오류 출력
            return; // 파일 저장 중단
        }
        ...
        if (!OrchardSaveBridge.TryCapture(saveData, out string orchardCaptureError)) // 과일나무 수집 (119일차)
        {
            Debug.LogError($"과일나무 저장 준비 실패\n{orchardCaptureError}", this); // 수집 오류 출력
            return; // 파일 저장 중단
        }
    ...
    public void LoadCurrentGame() // 저장된 게임 상태 불러오기
    {
        ...
        if (!NpcEventSaveBridge.TryRestore(saveData, out string npcEventRestoreError)) // NPC 이벤트 복원 (94일차, 의뢰보다 먼저 : 특별 의뢰 조건)
        {
            Debug.LogError($"NPC 이벤트 불러오기 실패\n{npcEventRestoreError}", this); // 복원 오류 출력
            return; // 전체 불러오기 중단
        }
    ...
    private SaveGameData CaptureCurrentState() // 현재 게임 상태를 저장 데이터로 변환
    {
        ...
        saveData.player.wetness = playerWetness.CurrentWetness; // 현재 젖음 수치
        saveData.player.hasTemperatureData = true; // 체온 저장 데이터 존재 표시
        saveData.player.temperature = playerTemperature.CurrentTemperature; // 현재 체온 수치 저장
